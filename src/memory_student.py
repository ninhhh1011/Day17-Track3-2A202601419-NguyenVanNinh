    ...
    def retrieve_episodic(self, user_id: str, query: str) -> str:
        ...
        for attempt in range(2):
            try:
                ...
                raw_eps = getattr(results, "episodes", []) or []
                real_eps = [
                    ep for ep in raw_eps
                    if not (getattr(ep, "thread_id", "") or "").startswith("eval-")
                    and getattr(ep, "role", "") != "Evaluation User"
                ]
                q_words = set(q.lower().split())
                sorted_eps = sorted(
                    real_eps or raw_eps,
                    key=lambda e: len(q_words & set((getattr(e, "content", "") or "").lower().split())),
                    reverse=True,
                )
                parts = [f"EPISODE: {e.content[:180]}" for e in sorted_eps if getattr(e, "content", None)]
